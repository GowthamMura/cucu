...
def patient_form():
    ...
    if request.method == "POST":
        admission_date_value = request.form.get("admission_date")
        discharge_date_value = request.form.get("discharge_date")
        is_discharged = request.form.get("is_discharged") == "yes"

        admission_date = (
            datetime.strptime(admission_date_value, "%Y-%m-%d").date() if admission_date_value else None
        )
        discharge_date = (
            datetime.strptime(discharge_date_value, "%Y-%m-%d").date() if discharge_date_value else None
        )
